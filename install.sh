...
if [ "$1" == "--install-service" ]; then
    ...
    systemctl is-active plex-xtream-bridge && echo -e "  ${GREEN}Running ✓${NC}" || echo -e "  ${RED}Not running ✗${NC}"
    echo ""
    ...
    echo "  bash $SCRIPT_DIR/install.sh --install-service"
fi

echo ""
echo -e "${CYAN}Access:${NC}"
echo "  Web: ${GREEN}http://$SERVER_IP:8080/admin${NC}"
echo "  Login: ${GREEN}admin123${NC} ${YELLOW}(change on first login!)${NC}"
echo ""
